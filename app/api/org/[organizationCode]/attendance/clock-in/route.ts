  ...
  try {
    ...
    if (!result.success) {
      return NextResponse.json(
        {
          success: false,
          error: result.error || 'Clock In verification failed.',
    ...
    return NextResponse.json({
      success: true,
      message: `Clock In verified successfully at ${result.record?.branch?.name || 'Branch'}.`,
    ...
    return NextResponse.json(
      { success: false, error: 'An unexpected error occurred during Clock In.' },
      { status: 500 }
    );
  }
}
